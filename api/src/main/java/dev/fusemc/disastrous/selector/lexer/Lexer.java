...
public class Lexer extends SourceReader<String> {
    ...
    private Lexeme.Number readNumber() throws ParseException {
        ...
        try {
            ...
            throw new ParseException("Encountered a malformed number literal in an event selector.", span);
        }
    }

    private Lexeme readIdentifier() throws ParseException {
        var buffer = new StringBuilder();
        var stamp = this.stamp();
        while (this.canRead()) {
            var c = this.peek();
        ...
        var span = this.span(stamp);
        return new Lexeme.Identifier(identifier, span);
    }
}
